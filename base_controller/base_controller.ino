        ...
        while(1) {
            ...
            while(1) {
               if(Serial.available() != 0){
                ...
                Serial.print("Global Setpoint: ");  Serial.println(setpoint);
                robot->front_right_motor_.forward();
                robot->front_right_motor_.stableRPM(setpoint);
                robot->back_left_motor_.forward();
                robot->back_left_motor_.stableRPM(setpoint);
                robot->front_left_motor_.forward();
                robot->front_left_motor_.stableRPM(setpoint);
                robot->back_right_motor_.forward();
                robot->back_right_motor_.stableRPM(setpoint);
                //robot->front_left_motor_.stableRPM(setpoint);
            }
